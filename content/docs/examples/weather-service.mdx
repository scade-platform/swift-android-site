...
description: Retrieving current temperature in a city
---
# Weather Service
...
The application uses two different APIs: Open-Meteo for weather data and OpenStreetMap Nominatim for geocoding city names to coordinates, demonstrating how to chain multiple API calls and handle different response formats.
...
In this code, we define a class `WeatherService` with two asynchronous methods: one for fetching the current temperature at given coordinates (`currentTemperature(latitude:longitude:response:)`) and another for geocoding a city name to coordinates and then fetching the temperature for those coordinates (`currentTemperature(city:response:)`) by calling the first function. The implementation of both functions is based on the standard functionality provided by the `Foundation` framework, such as `URL` for networking and `JSONSerialization` for parsing JSON data. Both functions use Swift's `async/await` syntax to handle asynchronous operations, and the `response` closure is called with the temperature and its units when the data is successfully fetched and parsed.
...
The full source code of the package is available on [GitHub](https://github.com/scade-platform/swift4j-examples/Packages/WeatherService).
